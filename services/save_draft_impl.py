...
import shutil
from infra.util import zip_draft, is_windows_path
from infra.oss import upload_to_oss
from typing import Dict, Literal
from infra.cache_service import draft_cache, get_task_status, update_tasks_cache, update_task_field, increment_task_field, update_task_fields, create_task
from infra.downloader import download_audio, download_file, download_image, download_video
...
def save_draft_background(draft_id, draft_folder, task_id):
    ...
    try:
        ...
        template_dir = "template" if settings.IS_CAPCUT_ENV else "template_jianying"
...
def download_script(draft_id: str, draft_folder: str = None, script_data: Dict = None, is_capcut: bool = None) -> Dict[str, str]:
    ...
    :param is_capcut: Whether to use CapCut template. If None, uses settings.IS_CAPCUT_ENV.
    ...
    # Use provided is_capcut flag or fallback to settings
    use_capcut = is_capcut if is_capcut is not None else settings.IS_CAPCUT_ENV

    logger.info(f"Starting to download draft: {draft_id} to folder: {draft_folder}")
    # Copy template to target directory
    template_path = os.path.join("./", 'template') if use_capcut else os.path.join("./", 'template_jianying')
